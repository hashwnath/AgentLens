...
def eval_val_sliding_window(
    ...
    batch_seqs = args.sw_eval_batch
    total_tokens = val_tokens.numel() - 1
    num_windows = max((total_tokens - seq_len) // stride + 1, 1)

    my_start = (num_windows * rank) // world_size
    my_end = (num_windows * (rank + 1)) // world_size
    ...
    with torch.inference_mode():
        for batch_start in range(my_start, my_end, batch_seqs):
            ...
            for w in range(batch_start, batch_end):
                ...
                wlen = we - ws
                chunk = val_tokens[ws:we + 1].to(device=device, dtype=torch.int64)
                if chunk.size(0) < seq_len + 1:
                    pad = torch.zeros(seq_len + 1 - chunk.size(0), device=device, dtype=torch.int64)
                    chunk = torch.cat([chunk, pad])
                    windows.append((chunk, wlen))
                else:
                    windows.append((chunk, wlen))

            x_batch = torch.stack([w[0][:-1] for w in windows])
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                logits = raw_model.forward_logits(x_batch)

            for i, (chunk, wlen) in enumerate(windows):
                tgt = chunk[1:wlen + 1] if wlen < seq_len else chunk[1:]
                s = 0 if batch_start + i == 0 else max(wlen - stride, 0)
                scored_logits = logits[i, s:wlen, :]
                scored_tgt = tgt[s:wlen]
                nll = F.cross_entropy(scored_logits.float(), scored_tgt, reduction="sum")
                loss_sum += nll.to(torch.float64)
                n_scored = scored_tgt.numel()
                token_count += n_scored

                prev_ids = chunk[s:wlen]
                tgt_ids = scored_tgt
...
# int5 for MLP weights, int6 for attention weights, fp16 for embeddings.

CONTROL_TENSOR_NAME_PATTERNS = tuple(
    pattern
    for pattern in os.environ.get(
        "CONTROL_TENSOR_NAME_PATTERNS",
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smeargate,gate",
    ).split(",")
    if pattern
)
...
def mixed_quantize_state_dict(state_dict: dict[str, Tensor]):
    ...
    for name, tensor in state_dict.items():
        ...
        if any(pattern in name for pattern in CONTROL_TENSOR_NAME_PATTERNS):
            ...
            stats["payload_bytes"] += tensor_nbytes(result[name])
            continue

        if any(pattern in name for pattern in FP16_KEEP_NAME_PATTERNS) or t.numel() <= 8192:
    ...
    return {"tensors": result, "meta": meta, "orig_dtypes": orig_dtypes}, stats

def dequantize_mixed_state_dict(obj: dict[str, object]) -> dict[str, Tensor]:
    result = obj["tensors"]
    meta = obj["meta"]
    orig_dtypes = obj["orig_dtypes"]
    out: dict[str, Tensor] = {}

    # Collect names that have .q/.scale pairs
    quant_names = {key[:-2] for key in result if key.endswith(".q")}
    ...
    for key, t in result.items():
        if key.endswith(".q"):
            ...
            q = result[name + ".q"]
            s = result[name + ".scale"]
            orig_dtype = getattr(torch, orig_dtypes[name])
            if s.ndim > 0:
                out[name] = (q.float() * s.float().view(q.shape[0], *([1] * (q.ndim - 1)))).to(dtype=orig_dtype).contiguous()
            else:
                out[name] = (q.float() * float(s.item())).to(dtype=orig_dtype).contiguous()
        elif key.endswith(".scale") and key[:-6] in quant_names:
            continue
        else:
            orig_dtype_str = orig_dtypes.get(key)
            if orig_dtype_str:
                out[key] = t.to(dtype=getattr(torch, orig_dtype_str)).contiguous()
            else:
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.num_kv_heads != self.num_heads:
            # Repeat KV heads to match Q heads (manual GQA for older GPUs like T4)
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        x = self._run_body(input_ids).reshape(-1, self.tok_emb.weight.size(1))
...
def main() -> None:
    ...
    # T4 (sm75) lacks flash attention; use mem-efficient or math fallback.
    cc = torch.cuda.get_device_capability(local_rank)
    has_flash = cc >= (8, 0)  # Ampere+
    ...
    # Add bigram embedding/projection to matrix_params for Muon
    if base_model.bigram is not None:
    ...
    log0(f"sdp_backends:cudnn=False flash={has_flash} mem_efficient={not has_flash} math={not has_flash}")
    ...
    # -----------------------------
    # MAGNITUDE PRUNING + SERIALIZATION + ROUNDTRIP VALIDATION
    # -----------------------------

    # Prune small weights to improve compressibility
    ...
    if master_process:
        ...
        log0(
            f"Serialized model mixed_quant+{_COMPRESSOR}: {quant_file_bytes} bytes "
    ...
    if args.eval_stride > 0:
        ...
        log0(
            ...
            f"eval_time:{1000.0 * (time.perf_counter() - t_sw):.0f}ms stride:{args.eval_stride}"
        )
